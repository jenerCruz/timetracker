...
(function () {
    ...
    function renderDashboard() {
        return `
            ...
            <div class="card p-6 text-center">
                ...
                <p class="mt-3 font-semibold text-indigo-600">¡Navega a la pestaña Registro para iniciar un turno!</p>
    ...
    async function updateClockButtonState() {
        ...
        const status = openEntry ? 'IN' : 'OUT'; // Si hay entrada abierta, el siguiente es OUT (para Clock Out)
        ...
        try {
            ...
            if (branch && branch.lat && branch.lng) {
                ...
                if (distance > geocercaRadio) {
                    ...
                    // return; // Si quieres FORZAR la geocerca. Por ahora solo se alerta.
                }
            }
    ...
    function renderSetup(branches = [], employees = []) {
        ...
        const branchListHtml = branches.map(b => 
            `<li class="p-2 border-b last:border-b-0 flex justify-between items-center">
                ${b.name} (${b.lat?.toFixed(3) || 'N/A'}, ${b.lng?.toFixed(3) || 'N/A'}) 
        ...
        return `
            ...
            <div class="card p-6">
                <h3 class="text-xl font-semibold mb-3">Configuración de Sincronización (Gist)</h3>
                <p class="text-xs text-gray-500 mb-4">La sincronización con Gist de GitHub garantiza la persistencia y centralización de los datos. Necesitarás tu Token de GitHub.</p>
                <div class="space-y-3">
    ...
    // ------------------ Lógica Gist (Placeholders de data.js) ------------------
    ...
    async function handleGistExport() {
        ...
        showMessage("Iniciando Exportación a Gist (Se necesita implementación real de la API de GitHub)...", 'info');
    ...
    async function handleGistImport() {
        ...
        showMessage("Iniciando Importación desde Gist (Se necesita implementación real de la API de GitHub)...", 'info');
        console.log("Token:", token, "Empleados Gist ID:", employeesGistId, "Sucursales Gist ID:", branchesGistId);
    }
    ...
    /** Función de inicio de la aplicación (interna al IIFE). */
